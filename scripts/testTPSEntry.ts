...
import { getEntries, getEntryByHash, addEntry, removeAll, getAll } from "../helpers/tps";
...
async function testGetEntry(hash, query) {
    ...
    console.assert((entries[0] as any).hash == (entry as any).hash, "Entries from getEntries is not the same as entry from getEntryByHash");
}
...
async function test() {
    ...
    console.assert(entry.hash != entry2.hash, "Example entries have the same hash");
