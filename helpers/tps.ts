...
function scramble(value: any) {
    ...
    return crypto.createHmac("sha256", config.auth.scoutingKeys[1])
        .update(value)
        .digest("hex")
        .substring(0, 16);
}

export function retrieveEntry(tps: TPSEntryType, teamNumber: string): TPSEntryType {
    ...
    privacyRules.forEach(rule => {
        ...
        if (rule.private && (!rule.teams.length || !rule.teams.includes(teamNumber))) {
            switch (rule.type) {
                case "scrambled":
    ...
    if (tps.privacy)
        delete tps.privacy; // do not pass the privacy rules to the requester
    return tps;
}
